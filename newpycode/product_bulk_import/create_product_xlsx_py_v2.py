...
def main():
    delete_exist=0
    Brandcode_prefix = 'demo-brand'
    NumOfBrands = 1

    args = sys.argv[1:]
    if not args:
      print ("usage: [--d] Number [Brandcode_prefix] [Number of Brands]");
      sys.exit(1)
    ...
    try:
        ...
        print ("usage: [--d] Number [Brandcode_prefix] [Number of Brands]. "+'Must specify a number')
        sys.exit(1)

    del args[0:1]
    if not args:
        print ('will use 1 brand and brand-name= "demo-brand"')
    else:
        Brandcode_prefix=str(args[0])
        try:
            NumOfBrands = int(args[1])
            if NumOfBrands <= 0:
                print ('\n\tIdiot')
                sys.exit(1)
        except:
    ...
    SkuPerBrand = int(math.floor(howmanynewproducts/NumOfBrands))
    brand_list=[]
    for n in range(NumOfBrands):
        for m in range(SkuPerBrand):
            brand_list.append(Brandcode_prefix+str(n+1))
    while len(brand_list) < howmanynewproducts:
        brand_list.append(Brandcode_prefix+str(NumOfBrands))
